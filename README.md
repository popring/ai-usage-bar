...
> **Claude Code** (multiple teams, plus every account managed by
> [claude-swap](https://github.com/realiti4/claude-swap) / cswap), **Codex**, and a self-hosted
> **LiteLLM** gateway. No telemetry, no third parties. Docs below are in Chinese;
> see [Privacy & Security](#隐私与安全) for exactly what it reads and sends.
...
菜单 → **设置…**（⌘,）打开配置面板：来源开关、LiteLLM 网关地址与 API Key、轮询间隔、菜单栏前缀，以及**一键添加 Claude team**。保存立刻生效，不用重启。
